...
public interface INotificationStore
{
    ...
    /// </summary>
    Task<List<NotificationSubscription>> GetSubscriptionsAsync(Guid?[] tenantIds, string notificationName, string entityTypeName, string entityId);

    /// <summary>
    /// Gets subscriptions for a user.
    /// </summary>
    Task<List<NotificationSubscription>> GetSubscriptionsAsync(UserIdentifier user);
    ...
    Task<List<UserNotificationWithNotification>> GetUserNotificationsWithNotificationsAsync(UserIdentifier user, UserNotificationState? state = null, int skipCount = 0, int maxResultCount = int.MaxValue, DateTime? startDate = null, DateTime? endDate = null);
    ...
    /// <param name="startDate">List notifications published after startDateTime</param>
    /// <param name="endDate">List notifications published before startDateTime</param>
    Task<long> GetUserNotificationCountAsync(UserIdentifier user, UserNotificationState? state = null, DateTime? startDate = null, DateTime? endDate = null);
    ...
    Task<UserNotificationWithNotification> GetUserNotificationWithNotificationOrNullAsync(Guid? tenantId, Guid userNotificationId);

    /// <summary>
    /// Inserts notification for a tenant.
    /// </summary>
    Task InsertTenantNotificationAsync(TenantNotification tenantNotificationInfo);
